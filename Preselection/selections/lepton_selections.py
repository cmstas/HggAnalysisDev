...
def select_electrons(events, photons, electrons, options, debug):
    ...
    if "dR_pho" in options["electrons"].keys():
        ...
        electron_cut = pt_cut & eta_cut & ip_xy_cut & ip_z_cut & id_cut 
        cut_diagnostics.add_cuts([pt_cut, eta_cut, ip_xy_cut, ip_z_cut, id_cut, electron_cut], ["pt", "eta", "ip_xy", "ip_z", "id", "m_egamma not in m_Z +/- 5 Gev", "all"])
...
def set_electrons(events, electrons, debug):
    ...
    electron_charge_padded = utils.pad_awkward_array(electrons.charge, 2, 0)
    ...
    events["ele2_tightId"] = electron_tightID_padded[:,1]
    events["ele1_charge"] = electron_charge_padded[:,0]
    events["ele2_charge"] = electron_charge_padded[:,1]
...
def set_muons(events, muons, debug):
    ...
    muon_charge_padded = utils.pad_awkward_array(muons.charge, 2, 0)
    ...
    events["muon1_charge"] = muon_charge_padded[:,0]
    events["muon2_charge"] = muon_charge_padded[:,1]

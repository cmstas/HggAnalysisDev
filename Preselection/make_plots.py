...
bin_svf = np.linspace(0,250,31)
bin_njet = np.linspace(0,5,6)
bin_nbjet = np.linspace(0,3,4)
bin_mZ = np.linspace(0,150,30)
bin_dR = np.linspace(0,5,20)

def process_event(evt_vars, genWeight): #, bjets):
    out_hists = {}
    out_hists["MET"] = Hist1D(ak.to_numpy(evt_vars.MET_pt), bins = bin_met, label="MET", weights = ak.to_numpy(genWeight))
    out_hists["tautau_SVFit"] = Hist1D(ak.to_numpy(evt_vars.tautauMass_SVFit), bins = bin_svf, label="di-tau m_SVfit", weights = ak.to_numpy(genWeight))
    out_hists["tautauA_SVFit"] = Hist1D(ak.to_numpy(evt_vars.tautauMassAll_SVFit), bins = bin_svf, label="di-tau All m_SVfit", weights = ak.to_numpy(genWeight))
    out_hists["tautauL_SVFit"] = Hist1D(ak.to_numpy(evt_vars.tautauMassLoose_SVFit), bins = bin_svf, label="di-tau Loose m_SVfit", weights = ak.to_numpy(genWeight))
    ...
    #out_hists["weight"] = ak.sum(genWeight)
    return out_hists

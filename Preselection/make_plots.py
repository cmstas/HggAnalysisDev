...
def process_diphoton(photons, gHidx, mgg, genWeight):
    ...
    out_hists["pho_pix_veto1"] = Hist1D(ak.to_numpy(getFromIdx(photons.pixelSeed, gHidx[:,0])), bins = bins_pixelSeed, weights = ak.to_numpy(genWeight), label = "pho_pix_veto1")
    out_hists["pho_pix_veto2"] = Hist1D(ak.to_numpy(getFromIdx(photons.pixelSeed, gHidx[:,1])), bins = bins_pixelSeed, weights = ak.to_numpy(genWeight), label = "pho_pix_veto2")

...
class Guided_Optimizer():
    def __init__(self, **kwargs):
        ...
        self.proc_ids = kwargs.get('proc_ids')
...
#            'signal' : ['ttHH_ggbb', 'ttHH_ggWW', 'ttHH_ggTauTau'],
#            'ttH' : ['ttH_M125'],
#            'data' : ['DiPhoton',  'GJets_HT-40To100','GJets_HT-100To200', 'GJets_HT-200To400', 'GJets_HT-400To600', 'GJets_HT-600ToInf', 'TTGG', 'TTGamma', 'TTJets', 'WGamma', 'ZGamma'],
#            'sm_higgs' : ['VBFH_M125', 'VH_M125', 'ggH_M125', 'ttH_M125']
#        }
...
#            'signal' : ['ttHH_ggbb', 'ttHH_ggWW', 'ttHH_ggTauTau'],
#            'ttH' : ['ttH_M125'],
#            'data' : ['DiPhoton',  'GJets/QCD(Data)', 'TTGG', 'TTGamma', 'TTJets', 'WGamma', 'ZGamma'],
#            'sm_higgs' : ['VBFH_M125', 'VH_M125', 'ggH_M125', 'ttH_M125']
#        }
...
#        }
    ...
    def proc_id_mapper(self):
        with open(self.proc_ids, 'r') as f:
            self.input_tags = json.load(f)
        sample_id_map = self.input_tags['sample_id_map']
        ...
        for group in self.process_dict.keys():
            for i in range(len(self.process_dict[group])):
                tmp_proc_dict[group].append(sample_id_map[self.process_dict[group][i]])

        self.process_dict = tmp_proc_dict
        return

    def process_selection(self, process):
        selection = " ("
        for i in range(len(self.process_dict[process])):
            selection += "process_id == " + str(self.process_dict[process][i])
            if i == (len(self.process_dict[process]) - 1):
                selection += ") "
            else:
                selection += " || "
        return selection
